...
'''
# --------------------------------------------
# training code for MDFPNet
# Baopeng Li (github: https://github.com/BP113)
# Mail: libaopeng@opt.ac.cn
# 03/Mar/2023
# --------------------------------------------
...
def main(json_path='options/train_ufpnet.json'):
        ...
        for i, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['checkpoint_test'] == 0:
                ...
                for test_data in test_loader:
                    ...
                    Ea_img = util.tensor2uint(visuals['Ea'])
                    Ha_img = util.tensor2uint(visuals['Ha'])

                    # -----------------------
                    # save estimated image E
                    # -----------------------
                    save_img_path = os.path.join(img_dir, '{:s}_{:d}.png'.format(img_name, current_step))
                    util.imsave(Ea_img, save_img_path)

                    # -----------------------
                    # calculate PSNR
                    # -----------------------

                    current_psnr = util.calculate_psnr(Ea_img, Ha_img, border=border)
                    current_ssim = util.calculate_ssim(Ea_img, Ha_img, border=border)

                    logger.info('{:->4d}--> {:>10s} | {:<4.2f}dB | {:<6.4f}'.format(idx, image_name_ext, current_psnr, current_ssim))
